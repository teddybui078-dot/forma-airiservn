...
Stages 1-4 run entirely in the browser so pose tracking and ghost comparison stay real-time. Stages 5-8 are server-side API routes so the Gemini and Fish Audio credentials never reach the client.
